...
impl Distribution {
    ...
    pub fn from_paths(kernel_path: &std::path::Path) -> Self {
        ...
        if path_str.contains("arch") || (path_str.contains("boot") && !path_str.contains("efi") && !path_str.contains("images")) {
            Distribution::Arch
        } else if path_str.contains("debian") || path_str.contains("ubuntu") || path_str.contains("casper") {
            Distribution::Debian
        } else if path_str.contains("fedora") || path_str.contains("images") || path_str.contains("pxeboot") {
            Distribution::Fedora
        } else {
...
pub trait IsoEnvironment: Send + Sync {
    ...
    fn validate_path(&self, base: &std::path::Path, path: &std::path::Path) -> Result<PathBuf, IsoError> {
        let resolved = path;
...
impl IsoEnvironment for OsIsoEnvironment {
    ...
    fn mount_iso(&self, iso_path: &std::path::Path) -> Result<PathBuf, IsoError> {
        ...
        let iso_name = iso_path.file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or("iso");
        ...
        let output = Command::new("mount")
            .args(["-o", "loop,ro", "-t", "iso9660", &iso_path.to_string_lossy(), &mount_point.to_string_lossy()])
            .output();
    ...
    fn unmount(&self, mount_point: &std::path::Path) -> Result<(), IsoError> {
        ...
        match umount_result {
            ...
            _ => {
                ...
                let fusermount = Command::new("fusermount").arg("-u").arg(mount_point).output();
                match fusermount {
                    Ok(out) if out.status.success() => {
                        let _ = std::fs::remove_dir(mount_point);
                        Ok(())
                    ...
                    _ => {
                        Err(IsoError::MountFailed(format!("Failed to unmount {:?}", mount_point)))
                    }
                }
            }
        }
...
impl<E: IsoEnvironment> IsoParser<E> {
    ...
    pub async fn scan_directory(&self, path: &std::path::Path) -> Result<Vec<BootEntry>, IsoError> {
        ...
        if entries.is_empty() {
            return Err(IsoError::NoBootEntries(validated_path.to_string_lossy().to_string()));
        }
    ...
    async fn extract_boot_entries(&self, mount_point: &Path, source_iso: &Path) -> Result<Vec<BootEntry>, IsoError> {
        let mut entries = Vec::new();
    ...
    fn try_arch_layout(&self, mount_point: &Path, source_iso: &Path) -> Result<Vec<BootEntry>, IsoError> {
        ...
        for entry in self.env.list_dir(&boot_dir)? {
            ...
            if name.starts_with("vmlinuz") {
                ...
                let mut initrd = boot_dir.join(format!("initrd.img{}", 
                ...
                if !self.env.exists(&initrd) {
                    initrd = boot_dir.join(format!("initrd{}", 
                        name.strip_prefix("vmlinuz").unwrap_or("")));
                }
                ...
                entries.push(BootEntry {
                    label: format!("Arch Linux {}", 
                        name.strip_prefix("vmlinuz").unwrap_or("").trim()),
                    kernel: kernel.strip_prefix(mount_point)
                        .map(|p| p.to_path_buf())
                        .map_err(|_| IsoError::Io(std::io::Error::new(
                            std::io::ErrorKind::InvalidData, "Kernel path escape")))?,
                    initrd: if has_initrd { Some(initrd) } else { None },
                    kernel_args: Some("archisobasedir=arch archiso_http_server=https://mirror.archlinux.org".to_string()),
                    distribution: Distribution::Arch,
                    source_iso: source_iso.file_name()
                        .and_then(|n| n.to_str())
                        .unwrap_or("unknown")
                        .to_string(),
                });
    ...
    fn try_debian_layout(&self, mount_point: &Path, source_iso: &Path) -> Result<Vec<BootEntry>, IsoError> {
        let mut entries = Vec::new();
            ...
            for entry in self.env.list_dir(search_dir)? {
                ...
                if name.starts_with("vmlinuz") {
                    ...
                    let kernel_args = if search_dir == &mount_point.join("casper") {
                        Some("boot=casper locale=en_US.UTF-8 keyboard-configuration/layoutcode=us".to_string())
                    } else {
                        None
                    };
                    ...
                    entries.push(BootEntry {
                        label: format!("Debian/Ubuntu {}", 
                            name.strip_prefix("vmlinuz").unwrap_or("").trim()),
                        kernel: kernel.strip_prefix(mount_point)
                            .map(|p| p.to_path_buf())
                            .map_err(|_| IsoError::Io(std::io::Error::new(
                                std::io::ErrorKind::InvalidData, "Kernel path escape")))?,
                        initrd: found_initrd.map(|p| p.strip_prefix(mount_point)
                            .map(|p| p.to_path_buf())
                            .map_err(|_| IsoError::Io(std::io::Error::new(
                                std::io::ErrorKind::InvalidData, "Initrd path escape"))))
                            .transpose()?,
                        kernel_args,
    ...
    fn try_fedora_layout(&self, mount_point: &Path, source_iso: &Path) -> Result<Vec<BootEntry>, IsoError> {
    ...
    fn process_fedora_isolinux(&self, isolinux_dir: &Path, mount_point: &Path, source_iso: &Path) -> Result<Vec<BootEntry>, IsoError> {
        let mut entries = Vec::new();
        ...
        for entry in self.env.list_dir(isolinux_dir)? {
            ...
            if name.starts_with("vmlinuz") {
                ...
                entries.push(BootEntry {
                    label: format!("Fedora (isolinux) {}", name.strip_prefix("vmlinuz").unwrap_or("").trim()),
                    kernel: kernel.strip_prefix(mount_point)
                        .map(|p| p.to_path_buf())
                        .map_err(|_| IsoError::Io(std::io::Error::new(
                            std::io::ErrorKind::InvalidData, "Kernel path escape")))?,
                    initrd: if self.env.exists(&initrd_path) {
                        Some(initrd_path.strip_prefix(mount_point)
                            .map(|p| p.to_path_buf())
                            .map_err(|_| IsoError::Io(std::io::Error::new(
                                std::io::ErrorKind::InvalidData, "Initrd path escape")))?)
                    } else { None },
...
mod tests {
        ...
        fn with_iso(distribution: Distribution) -> Self {
            ...
            match distribution {
                Distribution::Arch => {
                    ...
                    env.files.insert(mount_base.join("boot"), MockEntry::Directory(vec![
                        mount_base.join("boot/vmlinuz"),
                        mount_base.join("boot/initrd.img"),
                    ]));
                    env.files.insert(mount_base.join("boot/vmlinuz"), MockEntry::File);
                    env.files.insert(mount_base.join("boot/initrd.img"), MockEntry::File);
                }
                Distribution::Debian => {
                    // Debian: /install/vmlinuz, /casper/initrd.lz
                    env.files.insert(mount_base.join("install"), MockEntry::Directory(vec![
                        mount_base.join("install/vmlinuz"),
                    ]));
                    env.files.insert(mount_base.join("install/vmlinuz"), MockEntry::File);
                    env.files.insert(mount_base.join("casper"), MockEntry::Directory(vec![
                        mount_base.join("casper/initrd.lz"),
                        mount_base.join("casper/filesystem.squashfs"),
                    ]));
                    env.files.insert(mount_base.join("casper/initrd.lz"), MockEntry::File);
                    env.files.insert(mount_base.join("casper/filesystem.squashfs"), MockEntry::File);
                }
                Distribution::Fedora => {
                    // Fedora: /images/pxeboot/vmlinuz, /images/pxeboot/initrd.img
                    env.files.insert(mount_base.join("images"), MockEntry::Directory(vec![
                        mount_base.join("images/pxeboot"),
                    ]));
                    env.files.insert(mount_base.join("images/pxeboot"), MockEntry::Directory(vec![
                        mount_base.join("images/pxeboot/vmlinuz"),
                        mount_base.join("images/pxeboot/initrd.img"),
                    ]));
                    env.files.insert(mount_base.join("images/pxeboot/vmlinuz"), MockEntry::File);
                    env.files.insert(mount_base.join("images/pxeboot/initrd.img"), MockEntry::File);
                }
                Distribution::Unknown => {}
            }
            
            // Add ISO file in parent directory
            env.files.insert(PathBuf::from("/isos"), MockEntry::Directory(vec![
                PathBuf::from("/isos/test.iso"),
            ]));
            env.files.insert(PathBuf::from("/isos/test.iso"), MockEntry::File);
    ...
    impl IsoEnvironment for MockIsoEnvironment {
        fn list_dir(&self, path: &std::path::Path) -> std::io::Result<Vec<PathBuf>> {
            match self.files.get(path) {
                ...
                Some(MockEntry::File) => Err(std::io::Error::new(
                    std::io::ErrorKind::NotFound, "Not a directory")),
        ...
        fn mount_iso(&self, iso_path: &std::path::Path) -> Result<PathBuf, IsoError> {
            let mount_point = PathBuf::from(format!("/mock_mount/{}", 
    ...
    async fn test_arch_detection() {
        ...
        let mount_base = PathBuf::from("/mock_mount");
        let entries = parser.extract_boot_entries(&mount_base, &PathBuf::from("test.iso")).await.unwrap();
        ...
        assert!(entries.iter().any(|e| e.kernel.to_string_lossy().contains("vmlinuz")));
    }
    ...
    async fn test_debian_detection() {
        ...
        assert!(!entries.is_empty());
        assert!(entries.iter().any(|e| e.distribution == Distribution::Debian));
    }
    ...
    async fn test_fedora_detection() {
        ...
        assert!(!entries.is_empty());
        assert!(entries.iter().any(|e| e.distribution == Distribution::Fedora));
    }
    
    #[test]
    fn test_distribution_from_paths() {
        assert_eq!(Distribution::from_paths(PathBuf::from("/boot/vmlinuz").as_path()), Distribution::Arch);
        assert_eq!(Distribution::from_paths(PathBuf::from("/casper/vmlinuz").as_path()), Distribution::Debian);
        assert_eq!(Distribution::from_paths(PathBuf::from("/images/pxeboot/vmlinuz").as_path()), Distribution::Fedora);
    }
