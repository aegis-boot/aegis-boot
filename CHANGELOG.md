...
- **`aegis-boot fetch-image` cosign keyless auto-verify** (PR1 of [#235](https://github.com/williamzujkowski/aegis-boot/issues/235)) — adds a second integrity layer to `fetch-image` on top of the existing `--sha256` check. After download + sha256 verify, `fetch-image` now automatically downloads `<URL>.sig` + `<URL>.pem` from the same origin and runs `cosign verify-blob` against a hardcoded keyless identity bound to aegis-boot's own `release.yml` workflow (`^https://github\.com/williamzujkowski/aegis-boot/\.github/workflows/release\.yml@refs/tags/.+$` with the GitHub Actions OIDC issuer `https://token.actions.githubusercontent.com`). Graceful-degrades when signatures aren't published (curl 404 on the sidecar fetches → WARNING + proceed on sha256 alone) so operators pulling older releases or forks don't see a hard failure. On verification failure (signatures present but don't match), the image + both signature files are deleted and we exit 1 — same fail-closed contract as the sha256-mismatch path. New `--no-cosign` flag for air-gapped / offline operators whose Sigstore transparency-log lookup would fail regardless. New optional `cosign` row in `aegis-boot doctor`: `PASS` when present, `WARN` (not `FAIL`) when absent — doesn't dock the score below EXCELLENT for operators who never use `fetch-image`. Hardcoded regex is a security-critical constant; a drift-test locks the anchor + workflow + tag shape so a future refactor can't widen the identity match. No new crate dependencies (shells out to `curl` + `cosign`, both already host-deps). 6 new unit tests: `--no-cosign` parsing, default-enabled path (the "most automatic" contract), `sibling_with_suffix` helper for `.sig` / `.pem` path construction, identity-regex shape regression guard, OIDC issuer constant, pathless-basename fallback. Release.yml extension to actually publish `.img` + `.sig` + `.pem` for `aegis-boot.img` is the follow-up PR — this PR's code activates automatically when the signature assets ship.
