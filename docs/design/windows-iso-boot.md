...
### Revised recommendation: **just L1, pointing at Rufus** (final)

After the L1 + L2 recommendation landed, a second round of maintainer review (2026-04-24) surfaced that **L2 would reimplement what [Rufus](https://rufus.ie) already does** — Rufus is battle-tested on 100M+ installs, handles every Windows storage edge case we'd otherwise have to reimplement, and is the tool Windows operators already reach for.

Building our own Windows-installer flasher when Rufus exists is classic tool sprawl. L2 is dropped.

**Final scope: L1 only, pointing at Rufus.**

The rescue-tui tier-5 prose panel for Win11 ISOs becomes:
...
To try Linux instead, aegis-boot ships with:
  [list of Linux ISOs found on this stick, or a catalog-slug hint if none]
```

~30 LOC. Zero dependencies. Zero stick growth. No new subcommand. Points at a tool operators already trust.

### Broader anti-sprawl cleanup landing alongside L1

The same pushback prompted a handful of related simplifications shipped in this PR:

- README's Windows platform row + `docs/INSTALL.md` + `docs/CLI.md` all put **Rufus + pre-built `.img`** as the primary Windows path. `aegis-boot flash --direct-install` on Windows (#419 epic) is retained but demoted to "advanced path for CI automation + CLI-first operators" — most Windows operators don't need it.
- `winget-manifests/` scaffolding deleted. The templates existed to publish `aegis-boot.exe` via `winget install`; that channel is unnecessary when the recommended Windows install path is "download `.img`, open Rufus, done" with no CLI install involved.
- Epic #512 closed — the original 5-phase Windows-ISO-boot epic no longer has a real implementation target; only L1 (~30 LOC rescue-tui prose) is left as the implementation task.

### Meta: the vote missed the build-vs-buy question
